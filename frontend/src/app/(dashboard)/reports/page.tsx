...
import { PipelineBarChart, LeadStatusPieChart } from '@/components/charts/crm-charts';
import { BarChart3, FileText, Download, RefreshCw, CheckCircle2, TrendingUp, Users, Building2, Target, DollarSign, BarChart2, PieChart as PieIcon } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {!report ? (
        <div className="flex flex-col items-center justify-center p-12 border-2 border-dashed border-border rounded-lg bg-card text-center my-4">
          ...
          <h3 className="text-lg font-semibold text-foreground">Interactive Sample Report Generator</h3>
          <p className="text-sm text-muted-foreground max-w-md mt-1 mb-6">
            Click "Generate Sample Report" to run a live database analysis with visual bar charts and breakdown diagrams.
          </p>
          <button
          ...
          >
            ...
            <span>{generating ? 'Compiling Analytics...' : 'Generate Sample Report'}</span>
          </button>
        </div>
      ) : (
        <div className="space-y-6 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <StatCard title="Total Contacts" value={report.summary.totalContacts} icon={Users} />
            <StatCard title="Registered Companies" value={report.summary.totalCompanies} icon={Building2} />
            <StatCard title="Total Leads" value={report.summary.totalLeads} icon={Target} />
            <StatCard
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 p-6 rounded-lg border border-border bg-card shadow-xs space-y-4">
              <div className="flex items-center gap-2 border-b border-border pb-3">
                ...
                <h3 className="font-semibold text-foreground text-base">Pipeline Stage Financial Breakdown</h3>
              </div>
              <PipelineBarChart
                data={Object.entries(report.dealBreakdown).map(([stage, count]) => ({
                  stage,
                  value: count * 15000,
                }))}
              />
            ...
            <div className="p-6 rounded-lg border border-border bg-card shadow-xs space-y-4">
              <div className="flex items-center gap-2 border-b border-border pb-3">
                ...
                <h3 className="font-semibold text-foreground text-base">Lead Status Ratio</h3>
              </div>
              <LeadStatusPieChart
                data={Object.entries(report.leadBreakdown).map(([status, count]) => ({
                  status,
                  count,
                }))}
              />
          ...
          <div className="bg-card border border-border rounded-lg p-6 space-y-4 shadow-xs">
            <div className="flex items-center gap-2 border-b border-border pb-3">
              ...
              <h3 className="font-semibold text-foreground text-base">Key Executive Takeaways</h3>
            </div>
            <ul className="space-y-3">
              {report.insights.map((insight, i) => (
                <li key={i} className="flex items-start gap-2.5 text-sm text-foreground bg-secondary/30 p-3 rounded-md border border-border/40">
